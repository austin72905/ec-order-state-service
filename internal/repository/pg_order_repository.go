...
import (
	"context"
	"ec-order-state-service/internal/db"
...
func (r *PgOrderRepository) Save(order *domain.Order) error {
	...
	for attempt := 0; attempt < maxRetries; attempt++ {
		...
		err := pgx.BeginTxFunc(ctx, r.pool, pgx.TxOptions{
			...
			if existing.ID != "" {
				// 更新訂單狀態
				_, err = txQueries.UpdateOrderStatus(ctx, db.UpdateOrderStatusParams{
					ID:     order.ID,
				...
				// 創建新訂單
				_, err = txQueries.CreateOrder(ctx, db.CreateOrderParams{
					ID:        order.ID,
					Status:    string(order.Status),
					CreatedAt: timeToPgTimestamp(order.CreatedAt),
					UpdatedAt: timeToPgTimestamp(order.UpdatedAt),
				})
				if err != nil {
...
func (r *PgOrderRepository) GetOrdersByStatus(status domain.OrderStatus, limit, offset int) ([]*domain.Order, error) {
	...
	dbOrders, err := r.queries.GetOrdersByStatusWithPagination(ctx, db.GetOrdersByStatusWithPaginationParams{
...
func (r *PgOrderRepository) BatchUpdateOrderStatus(orderIDs []string, newStatus domain.OrderStatus, fromStatus domain.OrderStatus) error {
	...
	return r.queries.BatchUpdateOrderStatus(ctx, db.BatchUpdateOrderStatusParams{
		Status:   string(newStatus),
		Column2:  orderIDs,
		Status_2: string(fromStatus),
	})
}

// toDomainOrder 將資料庫模型轉換為領域模型
func toDomainOrder(dbOrder db.Order) *domain.Order {
	return &domain.Order{
		ID:        dbOrder.ID,
		Status:    domain.OrderStatus(dbOrder.Status),
		CreatedAt: pgTimestampToTime(dbOrder.CreatedAt),
		UpdatedAt: pgTimestampToTime(dbOrder.UpdatedAt),
	}
}

// timeToPgTimestamp 將 time.Time 轉換為 pgtype.Timestamp
...
// pgTimestampToTime 將 pgtype.Timestamp 轉換為 time.Time
